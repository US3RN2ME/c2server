...
namespace c2server::detail {
   ...
   void runServer(const ServerSettings& settings,
      ...
      const auto workerThreads = settings.workerThreads == 0
      ...
      auto sslContext = settings.ssl.enabled ? std::make_shared<ssl::context>(makeSslContext(settings.ssl))
                                             : std::shared_ptr<ssl::context>{};

      ShutdownSignalHandler shutdownSignals{
          ioc,
      ...
      net::co_spawn(net::make_strand(ioc),
                    listen(acceptor,
                           sslContext,
                           settings.ssl.allowPlainHttp,
                           [router = std::move(router)](const HttpRequest& req) {
                              return router->handle(req);
                           },
                           settings.requestBodyLimitBytes,
                           settings.requestTimeoutSeconds),
                    net::detached);

      log::info("Server running on {}:{} with {} worker thread(s), ssl {}",
                settings.host,
                settings.port,
                workerThreads,
